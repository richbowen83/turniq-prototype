...
export async function updateTurnForOrg(orgId, turnId, patch = {}) {
  const supabase = getSupabaseServerClient();
  ...
  const dbRow = {
    ...
    name: nextPayload.name || null,
    market: nextPayload.market || null,
    current_stage: nextPayload.currentStage || null,
    turn_status: nextPayload.turnStatus || null,
    turn_owner: nextPayload.turnOwner || null,
    vendor: nextPayload.vendor || null,

    projected_completion: nextPayload.projectedCompletion || null,
    open_days: Number.isFinite(Number(nextPayload.openDays))
      ? Number(nextPayload.openDays)
      : null,
    days_in_stage: Number.isFinite(Number(nextPayload.daysInStage))
      ? Number(nextPayload.daysInStage)
      : null,
    risk: Number.isFinite(Number(nextPayload.risk))
      ? Number(nextPayload.risk)
      : null,
    readiness: Number.isFinite(Number(nextPayload.readiness))
      ? Number(nextPayload.readiness)
      : null,
  ...
  const { data, error } = await supabase
    .from("turns")
    .upsert(dbRow, { onConflict: "org_id,id" })
    .select()
    .single();
