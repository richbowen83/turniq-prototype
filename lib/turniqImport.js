...
initialEstimatedRentReadyDate: [
  "Initial Estimated Rent Ready Date",
  "initial_ecd",
],
...
region: ["Region", "region_name"],
  estimateCost: ["Estimate Cost", "Estimated Cost", "projectedCost"],
  approvedCost: ["Approved Cost"],
  moiStatus: ["Moi Status"],
  rriStatus: ["Rri Status"],
  rriInspectionResult: ["Rri Inspection Result"],
  lockboxStatus: ["Lockbox Status"],
  latestInvestorApprovalRequestedAt: ["Latest Investor Approval Requested At"],
  latestInvestorApprovalResponseAt: ["Latest Investor Approval Response At"],
  daysSinceLatestLockboxAccess: ["Days Since Latest Lockbox Access"],
  woEstimatesCount: ["Wo Estimates Count"],
  woApprovedCount: ["Wo Approved Count"],
  region: ["Region"],
  city: ["City"],
  state: ["State"],
...
export function mapRawRowToTurnIQTurn(row, index = 0) {
  ...
  const turn = {
    ...
    inspection_status:
      cleanString(row["Rri Inspection Result"]) || cleanString(row["Moi Status"]),
    approval_status:
