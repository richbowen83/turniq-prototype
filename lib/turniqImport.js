...
export function mapRawRowToTurnIQTurn(row, index = 0) {
  ...
  // --- DATE + FIELD NORMALIZATION (single source of truth) ---
  ...
  const initialEstimatedCompletion = parseDate(
    row["Initial Estimated Rent Ready Date"]
  );

  const openDays = toNumber(row["Days Open"]);
  const daysInStage = toNumber(row["Current Days in Stage"]);
  ...
  const approvalRequestedAt = parseDate(
    row["Latest Investor Approval Requested At"]
  );
  const approvalResponseAt = parseDate(
    row["Latest Investor Approval Response At"]
  );

  const lockboxStatus = cleanString(row["Lockbox Status"]);
  const accessDaysSince = toNumber(row["Days Since Latest Lockbox Access"]);

  const woEstimatesCount = toNumber(row["Wo Estimates Count"]);
  ...
  const isForSale = toBoolean(row["Is for Sale"]);
  ...
  const turn = {
    ...
    initialProjectedCompletion: initialEstimatedCompletion,
    initial_estimated_completion: initialEstimatedCompletion,
    ...
    // --- INSPECTIONS ---
    moi_status: moiStatus,
    rri_status: rriStatus,
    rri_result: rriInspectionResult,

    inspection_status: rriInspectionResult || moiStatus,
    ...
    wo_estimates_count: woEstimatesCount,
